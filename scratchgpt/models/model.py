...
# config = BaseConfig()  # or BaseConfig() for smaller model
...
class MaskedMultiHeadAttention(nn.Module):
    def __init__(self, head_size, num_heads, n_embd, block_size):
        super(MaskedMultiHeadAttention, self).__init__()
        self.heads = nn.ModuleList([MaskedAttentionHead(head_size, n_embd, block_size) for _ in range(num_heads)])
...
class Block(nn.Module):
    def __init__(self, n_embd, n_heads, block_size):
        ...
        self.sa = MaskedMultiHeadAttention(head_size, n_heads, n_embd, block_size)
...
class Model(nn.Module):
    def __init__(self, vocab_size, block_size= 8, n_embd=32, nb_heads=4, nb_layers=4):
        ...
        self.sa_blocks = nn.Sequential(*[Block(n_embd, nb_heads, block_size) for _ in range(nb_layers)]) # 4 blocks of self attention
